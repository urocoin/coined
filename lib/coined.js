...
var bcoin = require('bcoin-uro')
  , bn = require('bcoin-uro/node_modules/bn.js')
  , async = require('bcoin-uro/node_modules/async');
...
function Coined(options) {
  ...
  this.walletPath = options.walletPath || options.wallet
    || './uro-wallet.json'//process.env.HOME + '/.coined/wallet.json';

  this.addr = null;
  this.dust = 100000; //0.001 Uro
  this.fee = 100000; //0.001 Uro
...
Coined.prototype.init = function() {
  ...
  this.pool.on('tx', this._poolOnTX = function(tx, peer) {
    var hash = tx.hash('hex');

    if (self.pending[hash]) {
      if (!tx.block) {
        if (peer.lastBlock) {
          tx.ts = peer.lastBlock.ts;
          tx.block = peer.lastBlock.hash('hex');
        } else {
          return;
        }
      }
